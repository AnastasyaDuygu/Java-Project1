...
public class UniLibrarySys {
	...
	public static void addPerson(String nameSurname, int personId, String borrowDate, String phoneNo, String email, String memberType) {
		Borrower p;
			if(memberType.equalsIgnoreCase("Guest")) {
				...
				p = new Guest(nameSurname, personId, phoneNo, email, null);
				personList.add(p);
				...
				p = new UniMember(nameSurname, personId, phoneNo, email, null);
				personList.add(p);
			}else
	...
	public static Borrower searchPerson(int personId){
		Borrower b;
		for(int i = 0 ; i < personList.size(); i++) {
			if(personList.get(i).getPersonId() == personId)
	...
	public static Electronic getElectronic(int bookId) {
		Iterator<Book> it = bookSet.iterator();
		Book b;
		while(it.hasNext()) {
			b = it.next();
			if(b instanceof Electronic && b.getBookId() == bookId) {
				return (Electronic) b;
			}
		}
		return null;
	...
	public static void addOwnedBook(int bookId, int personId) {
		
		for(int i = 0; i < personList.size(); i++) {
			if(personList.get(i).getPersonId() == personId)
				personList.get(i).setOwnedBook(getHardcopy(bookId));
		}
