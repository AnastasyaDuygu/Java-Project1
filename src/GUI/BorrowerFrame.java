...
import BookInheritance.Electronic;
...
public class BorrowerFrame extends JFrame {
	 ...
	 * Create the frame.
	 */
	public BorrowerFrame(LoginFrame lf) {
		login = lf;
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(textArea.getText().isEmpty()) {
					...
					if(textField.getText().isEmpty()) {
						...
						int bookId = Integer.parseInt(textField.getText());
						//System.out.println(login.getBorrowerId());
						UniLibrarySys.addOwnedBook(bookId, login.getBorrowerId());
						if(UniLibrarySys.getHardcopy(bookId)==null) {
							Electronic el = UniLibrarySys.getElectronic(bookId);
							textArea.setText("You cannot borrow electronic books!\nHere is the link of the book: "+ el.getLink());
						}else {
